"""
STEP 3 — Generate Abstractive Summaries using Fine-tuned Model
===============================================================
Loads the fine-tuned model from 'bangla_resume_summarizer/' (output of Step 2),
runs inference on all resumes in raw_resumes/, saves final CSV.

Prerequisites:
    - bangla_resume_summarizer/ folder must be in the same directory
...
# ─── CONFIG ──────────────────────────────────────────────────────────────────
MODEL_DIR   = "bangla_resume_summarizer"      # fine-tuned model from Step 2
RAW_FOLDER  = "raw_resumes"
OUTPUT_PATH = "MT5_Abstractive_Summaries.csv"
LANG_TOKEN  = "<2bn>"
...
def generate_summary(sentences: list) -> str:
    """Run fine-tuned mT5 inference on a list of sentences."""
    if not sentences:
        return "তথ্য নেই।"

    input_text = LANG_TOKEN + " " + " । ".join(sentences[:4])
...
if not os.path.exists(MODEL_DIR):
    print(f"Error: '{MODEL_DIR}' not found. Run Step 2 first.")
    exit(1)
